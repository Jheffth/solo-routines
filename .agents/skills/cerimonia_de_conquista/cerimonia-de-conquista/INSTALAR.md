...
5. creditar XP **pelo motor** (nunca `usuario.xp_total += x` direto);
6. acumular múltiplos level-ups numa cerimônia só;
7. rodar `node --check` no final.
...
└── referencia/
    ...
    ├── ascensao.js             Ascensao: level-up com acúmulo (retorna Promise)
    ├── ascensao.css            fenda, pilar de luz, ondas, runas, tremor
    ├── motor_recompensa.py     creditar_bonus / recalcular_nivel (regra do motor)
